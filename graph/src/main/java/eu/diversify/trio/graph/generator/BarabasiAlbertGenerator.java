...
public class BarabasiAlbertGenerator implements GraphGenerator {
        ...
        while (index < nodeCount()) {
            ...
            switch (action) {
                case NEW_NODE_WITH_OUTGOING_EDGE:
                    graph.connect(node(index), select(graph, TARGET));
                    index++;
                    break;
                case NEW_NODE_WITH_INCOMING_EDGE:
                    graph.connect(select(graph, SOURCE), node(index));
                    index++;
                case NEW_EDGE:
                    graph.connect(select(graph, SOURCE), select(graph, TARGET));
                    break;
            }
        }
        return graph;
    ...
    private Node select(Graph graph, boolean role) {
